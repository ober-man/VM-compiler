...
void LoopAnalyzer::findLoopsRec(BasicBlock* bb, BasicBlock* prev_bb)
{
...
void LoopAnalyzer::fillLoopRec(Loop* loop, BasicBlock* bb)
{
    if (bb->isMarked(gray_mrk))
    {
        // we reach a header or already visited bb
        return;
    }
...
{
    Loop* root_loop = new Loop{graph->getFirstBB()};
